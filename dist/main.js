...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (headerMount)
        headerMount.innerHTML = headerComponent.render();
    const heroMount = document.getElementById('home');
    if (heroMount)
        heroMount.innerHTML = heroComponent.render();
    const aboutMount = document.getElementById('about');
    if (aboutMount)
        aboutMount.innerHTML = aboutComponent.render();
    const servicesMount = document.getElementById('services');
    if (servicesMount)
        servicesMount.innerHTML = servicesComponent.render();
    const processMount = document.getElementById('process');
    if (processMount)
        processMount.innerHTML = processComponent.render();
    const projectsMount = document.getElementById('projects');
    if (projectsMount)
        projectsMount.innerHTML = projectsComponent.render();
    const contributionsMount = document.getElementById('contributions');
    if (contributionsMount)
        contributionsMount.innerHTML = contributionsComponent.render();
    const contactMount = document.getElementById('contact');
    if (contactMount)
        contactMount.innerHTML = contactComponent.render();
    const footerMount = document.getElementById('footer-mount');
    if (footerMount)
        footerMount.innerHTML = footerComponent.render();
    const terminalMount = document.getElementById('terminal-mount');
    if (terminalMount)
        terminalMount.innerHTML = terminalComponent.render();
